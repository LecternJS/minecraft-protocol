const { endianToggle } = require('../../Util/Util');

function pingpong(client, server, { beforePing = null }) {
	...
	function onPing() {
		...
		function answerToPing(err, pingResponse) {
			if (err) return;
	...
	function onLegacyPing(packet) {
		...
		function sendPingResponse(responseString) {
			function utf16be(string) {
				return endianToggle(Buffer.from(string, 'utf16le'), 16);
			}

			const responseBuffer = utf16be(responseString);

			const { length } = responseString;
			// UCS2 characters, not bytes
