...
const { bufferEquals } = require('../../Util/Util');
const PluginChannels = require('../../Client/PluginChannels');

function login(client, server, options) {
	const yggdrasilServer = yggdrasil.server({ agent: options.agent });
	const {
		'online-mode': onlineMode = true,
		kickTimeout = 30 * 1000,
		errorHandler: clientErrorHandler = (player, err) => player.end(err)
	} = options;
	...
	function onEncryptionKeyResponse(packet) {
		...
		try {
			...
			if (!bufferEquals(client.verifyToken, verifyToken)) {
		...
		function verifyUsername() {
			yggdrasilServer.hasJoined(client.username, serverId, sharedSecret, client.publicKey, (err, profile) => {
				if (err) {
	...
	function javaUUID(strang) {
		const hash = crypto.createHash('md5');
		hash.update(strang, 'utf8');
	...
	function loginClient() {
		...
		if (client.protocolVersion >= 27) {
			// 14w28a (27) added whole-protocol compression (http://wiki.vg/Protocol_History#14w28a), earlier versions per-packet compressed TODO: refactor into minecraft-data
